...
% sequence_type = SequenceType.Hercforce;
...
[points, amps] = point_grid( [0]/1000, [0]/1000, [10:5:80]/1000);
...
forces_array = zeros(max_length,row_count,no_transmits);
herc_array = forces_array;

% Pad each transmit with 0s so they are the same length.
for T = 1:no_transmits
    padded_data = zeros(max_length, row_count);
    padded_data(1:signal_lengths(T), :) = all_scans{T};
    
    forces_array(:,:,T) = padded_data();
end



%% Forces Decoding
data_array_force = reshape(forces_array, [], no_transmits);
...
forces_cell = mat2cell(data_array_force, max_length, column_count, ones(1, no_transmits));

%% Forces Volume configuration
x_range = [-10, 10]/1000;
y_range = [0 0]/1000;
z_range = [5, 80]/1000;

resolution = pitch; % Spatial voxel size
...
if cuda_beamform == true
    cuda_cell = cuda_processing_f2(forces_array,tx_config,1,vol_config, f_number, sequence_type);
    forces_image_raw = cuda_cell{1};
else
    forces_image_raw = beamform_volume(tx_config, vol_config, forces_cell, rx_element_locs,...
...
%% Hercforces Beamforming

% Take the forces width but beamform a cube
herc_vol_config = vol_config;
herc_vol_config.y_range = rx_element_locs{1}(1,:);
herc_vol_config.y_count = length(herc_vol_config.y_range);
herc_vol_config.y_min = herc_vol_config.y_range(1);
herc_vol_config.y_max = herc_vol_config.y_range(end);
herc_vol_config.y_mid = herc_vol_config.y_range(round(row_count/2));

% bp.das_shader_id = 0; % Forces
% bp.das_shader_id = 2; % Hercules
% 5 = hercforces
sequence_type = 5;
cuda_cell = cuda_processing_f2(forces_array,tx_config,1,herc_vol_config, f_number, sequence_type);
herc_volume_raw = cuda_cell{1};
...
processed_herc_volume = process_volume(herc_volume_raw,dynamic_range);

herc_slice = squeeze(processed_herc_volume(64,:,:)).';
...
subplot 212
colormap("gray");
imagesc(herc_vol_config.y_range*1000, vol_config.z_range*1000, herc_slice);
subtitle('Hercules', 'FontSize',14);
axis("image")
clim([-dynamic_range, 0]); 
xlabel("Lateral Distance (mm)");
ylabel("Axial Distance (mm)");
